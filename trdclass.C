...
//      b_branchname->GetEntry(ientry); //read only this branch
   ...
   TCanvas *c0 = new TCanvas("DISPLAY",c0Title,200,200,1300,900);
   ...
   //
   hCCCor_u = new TH2F("hCCCor_u"," Cherenkov Calorimeter Corr ; Upstream ; Cal ",400,0.5,4095.5,400,0.5,4095.5);
   hCCCor_dout = new TH2F("hCCCor_dout"," Cherenkov Calorimeter Corr ; Downstream out ; Cal ",400,0.5,4095.5,400,0.5,4095.5);
   hCCor_ud = new TH2F("hCCor_ud"," Cherenkov ud Corr ; Upstream ; Downstream out ",400,-0.5,4095.5,400,0.5,4095.5);
   //
   f125_el = new TH1F("f125_el","GEM f125 peak amp for electrons",100,0.,4096);
   f125_pi = new TH1F("f125_pi","GEM f125 peak amp for pions",100,0.,4096);
   mmg_f125_el = new TH1F("mmg_f125_el","MMG-1 f125 peak amp for electrons",100,0.,4096);
   mmg_f125_pi = new TH1F("mmg_f125_pi","MMG-1 f125 peak amp for pions",100,0.,4096);

   f125_el_amp2d = new TH2F("f125_el_amp2d","GEM amp (ch,time) for electrons",200,0.5,200.5,240,0.5,240.5);
   f125_pi_amp2d = new TH2F("f125_pi_amp2d","GEM amp (ch,time) for pions",200,0.5,200.5,240,0.5,240.5);
   mmg_f125_el_amp2d = new TH2F("mmg_f125_el_amp2d","MMG-1 amp (ch,time) for electrons",200,0.5,200.5,240,0.5,240.5);
   mmg_f125_pi_amp2d = new TH2F("mmg_f125_pi_amp2d","MMG-1 amp (ch,time) for pions",200,0.5,200.5,240,0.5,240.5);
   urw_f125_el_amp2d = new TH2F("urw_f125_el_amp2d","uRWELL amp (ch,time) for electrons",200,0.5,200.5,240,0.5,240.5);
   urw_f125_pi_amp2d = new TH2F("urw_f125_pi_amp2d","uRWELL amp (ch,time) for pions",200,0.5,200.5,240,0.5,240.5);
   f125_el_clu2d = new TH2F("f125_el_clu2d","GEM cluster amp (ch,time) for electrons",200,0.5,200.5,240,0.5,240.5);
   f125_pi_clu2d = new TH2F("f125_pi_clu2d","GEM cluster amp (ch,time) for pions",200,0.5,200.5,240,0.5,240.5);
   mmg_f125_el_clu2d = new TH2F("mmg_f125_el_clu2d","MMG-1 cluster amp (ch,time) for electrons",200,0.5,200.5,240,0.5,240.5);
   mmg_f125_pi_clu2d = new TH2F("mmg_f125_pi_clu2d","MMG-1 cluster amp (ch,time) for pions",200,0.5,200.5,240,0.5,240.5);
   urw_f125_el_clu2d = new TH2F("urw_f125_el_clu2d","uRWELL cluster amp (ch,time) for electrons",200,0.5,200.5,240,0.5,240.5);
   urw_f125_pi_clu2d = new TH2F("urw_f125_pi_clu2d","uRWELL cluster amp (ch,time) for pions",200,0.5,200.5,240,0.5,240.5);
            ...
            bool electron=false;
     		...
     		for (int i=0;i<f250_wraw_count; i++) { // --- fadc250 channels loop
			     ...
			     , f250_wraw_samples_index->at(i), f250_wraw_samples_count->at(i));
       		    ...
       		    int amax=0; int tmax=9; // -- max amplitude & time in a sample
