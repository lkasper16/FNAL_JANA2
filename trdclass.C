...
void trdclass::Loop() {
  ...
  for (jentry=0; jentry<nentries;jentry++) {
    ...
    for (int i=0;i<f125_pulse_count; i++) {
      ...
      if(electron){
	if ( (slot<6) || (slot==6 && chan<24) ) { f125_el_amp2d->Fill(time,dchan,amp);  if (!(jentry%1000)) f125_el_evt->Fill(time,dchan,amp); } 
	...
	if ((slot<6)||(slot==6&&chan<24)) { f125_pi_amp2d->Fill(time,dchan,amp); if (!(jentry%1000)) f125_pi_evt->Fill(time,dchan,amp); } 
...
//#define USE_125_RAW
  ...
  cout<<" total events jentry= "<<jentry<<endl;
  ...
  //open
  TFile* fOut;
  char rootFileName[256]; sprintf(rootFileName, "FNAL_JANA2/RunOutput/Run%d_Output.root", RunNum);
